...
def scan_binary_markets(
    ...
    binary_markets = [m for m in markets if not m.neg_risk and m.active]
    ...
    books = get_orderbooks(client, all_token_ids)
...
def _check_buy_arb(
    ...
    max_sets = min(yes_ask.size, no_ask.size)
    if max_sets <= 0:
        return None

    gross_profit = profit_per_set * max_sets

    # Gas cost: 2 orders on Polygon
    gas_cost_wei = 2 * gas_per_order * gas_price_gwei * 1e9
    gas_cost_matic = gas_cost_wei / 1e18
    gas_cost_usd = gas_cost_matic * 0.50  # rough MATIC/USD, conservative
    ...
    return Opportunity(
        ...
        legs=(
            LegOrder(token_id=market.yes_token_id, side=Side.BUY, price=yes_ask.price, size=max_sets),
            LegOrder(token_id=market.no_token_id, side=Side.BUY, price=no_ask.price, size=max_sets),
        ),
...
def _check_sell_arb(
    ...
    max_sets = min(yes_bid.size, no_bid.size)
    if max_sets <= 0:
        return None
    ...
    net_profit = gross_profit - gas_cost_usd
    required_capital = 1.0 * max_sets  # cost basis is $1 per set (what we paid to acquire)
    roi_pct = (net_profit / required_capital * 100) if required_capital > 0 else 0
    ...
    return Opportunity(
        ...
        event_id=market.event_id,
        legs=(
            LegOrder(token_id=market.yes_token_id, side=Side.SELL, price=yes_bid.price, size=max_sets),
            LegOrder(token_id=market.no_token_id, side=Side.SELL, price=no_bid.price, size=max_sets),
        ),
        expected_profit_per_set=profit_per_set,
        max_sets=max_sets,
        gross_profit=gross_profit,
