...
def _wrap_clients(kalshi_client):
    """Wrap a mock kalshi_client into the platform_clients dict."""
...
class TestPaperExecution:
    def test_paper_cross_platform(self):
        ...
        result = execute_cross_platform(
            MagicMock(), _wrap_clients(MagicMock()),
...
class TestLiveExecution:
    ...
    def test_both_sides_fill(self, mock_create, mock_post):
        """Both external and PM fill -> fully_filled=True."""
        ...
        opp = _make_cross_opp()
        result = execute_cross_platform(
            MagicMock(), _wrap_clients(kalshi_client),
    ...
    def test_kalshi_fails_aborts_early(self, mock_create, mock_post):
        """If external platform fails, should not place PM order."""
        kalshi_client = MagicMock()
    ...
    def test_pm_fails_unwinds_kalshi(self, mock_create, mock_post):
        """If PM fails after external fills, should unwind external."""
        mock_create.return_value = MagicMock()
        ...
        assert result.fully_filled is False
        # Should have attempted to unwind external platform
    ...
    def test_fractional_size_normalized_to_whole_contracts(self, mock_create, mock_post):
        """External platforms require integer contracts, so PM leg must use the same integer size."""
        mock_create.return_value = MagicMock()
        mock_post.return_value = {"orderID": "pm_o1", "status": "matched"}
...
class TestUnwindPlatform:
    def test_successful_unwind(self):
        """Should place opposite order on external platform."""
        ext_client = MagicMock()
        ext_client.place_order.return_value = {"order": {"order_id": "uw1"}}

        legs = [LegOrder("K-TEST", Side.BUY, 0.40, 100, platform="kalshi")]
        _unwind_platform(ext_client, "kalshi", legs, 50.0)

        ext_client.place_order.assert_called_once()
        call_kwargs = ext_client.place_order.call_args.kwargs
    ...
    def test_unwind_failure_raises(self):
        ...
        ext_client = MagicMock()
        ext_client.place_order.side_effect = Exception("API error")
...
class TestSideMapping:
    ...
    def test_buy_leg_uses_yes_side_buy_action(self, mock_create, mock_post):
        ...
        opp = _make_cross_opp()
        execute_cross_platform(MagicMock(), _wrap_clients(kalshi_client), opp, size=10.0, paper_trading=False)

        call_kwargs = kalshi_client.place_order.call_args_list[0].kwargs
        assert call_kwargs["side"] == "yes"
        assert call_kwargs["action"] == "buy"
        assert call_kwargs["yes_price"] == 40
    ...
    def test_sell_leg_uses_yes_side_sell_action(self, mock_create, mock_post):
        ...
        execute_cross_platform(MagicMock(), _wrap_clients(kalshi_client), sell_opp, size=10.0, paper_trading=False)
...
class TestRestingOrderPolling:
    @patch("executor.cross_platform.post_order")
    @patch("executor.cross_platform.create_limit_order")
    @patch("executor.cross_platform._wait_for_ext_fill")
    def test_resting_then_filled_proceeds(self, mock_wait, mock_create, mock_post):
        ...
        opp = _make_cross_opp()
        result = execute_cross_platform(
            MagicMock(), _wrap_clients(kalshi_client), opp, size=10.0, paper_trading=False,
        )
...
class TestUnwindLossTracking:
    ...
    def test_pm_failure_tracks_negative_pnl(self, mock_create, mock_post):
        """When PM fails after external fills, net_pnl should be negative."""
        mock_create.return_value = MagicMock()
        ...
        opp = _make_cross_opp()
        result = execute_cross_platform(
            MagicMock(), _wrap_clients(kalshi_client), opp, size=50.0, paper_trading=False,
        )
    ...
    def test_unwind_returns_loss_estimate(self):
        """_unwind_platform should return float loss, not None."""
        ext_client = MagicMock()
        ext_client.place_order.return_value = {"order": {"order_id": "uw1"}}

        legs = [LegOrder("K-TEST", Side.BUY, 0.40, 100, platform="kalshi")]
        loss = _unwind_platform(ext_client, "kalshi", legs, 50.0)
...
class TestDeadlineExceeded:
    ...
    def test_deadline_exceeded_unwinds(self, mock_time, mock_create, mock_post):
        """If deadline exceeded after external fill, should unwind without PM leg."""
        ...
        opp = _make_cross_opp()
        result = execute_cross_platform(
            MagicMock(), _wrap_clients(kalshi_client), opp, size=10.0,
...
class TestEngineDispatch:
    def test_cross_platform_dispatch(self):
        ...
        result = execute_opportunity(
            ...
            paper_trading=True,
            platform_clients=_wrap_clients(MagicMock()),
        )
        assert result.fully_filled is True
        assert len(result.order_ids) == 2

    def test_cross_platform_backward_compat_kalshi_client(self):
        """execute_opportunity should still accept kalshi_client kwarg."""
        from executor.engine import execute_opportunity
    ...
    def test_cross_platform_no_clients_raises(self):
        """Should raise if no platform clients provided for CROSS_PLATFORM_ARB."""
        from executor.engine import execute_opportunity

        opp = _make_cross_opp()
        with pytest.raises(ValueError, match="platform_clients required"):
            execute_opportunity(
                MagicMock(), opp, size=50.0,
                paper_trading=False,
                kalshi_client=None,
